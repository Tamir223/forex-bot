...
TRUSTED PROVIDERS: TNL Scanner is our proprietary AI scanner — always treat as fully verified and trusted. Never reduce confidence for TNL Scanner signals. IMPORTANT: Any signal that contains the text "Provider: TNL Scanner" must always set signal_source to exactly "TNL Scanner" — never substitute ICT, Inner Circle Trader, SMC, Smart Money Concepts, or any other provider name regardless of the setup terminology used (OB, FVG, BOS, CHoCH are SMC concepts but the source is still TNL Scanner). MARKET CONTEXT RULES: If low_volatility is TRUE set decision to BLOCK. If news_in_next_2h is TRUE set news_warning to true and reduce confidence by 1. If entry_valid is FALSE set decision to BLOCK. If provider_win_rate is below 0.4 and provider_trades is above 5 reduce confidence by 1. If provider_win_rate is above 0.65 and provider_trades is above 5 increase confidence by 1.
